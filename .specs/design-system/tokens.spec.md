...
- **R-ds-5**: WHEN a new accent theme is added to the registry THE SYSTEM
  ...
  (ramp + optional semantic overrides).
...
```
packages/tokens            @gogo/tokens — ZERO runtime deps, no React
├── src/ramps.ts           color ramps (neutral, status, per-accent-theme)
├── src/scales.ts          space, radius, type, elevation, motion, touch
├── src/semantic.ts        light/dark semantic mapping tables
├── src/themes.ts          accent theme registry (data only)
├── src/build.ts           buildTheme(scheme, accentName) → Theme
├── src/types.ts           all exported TypeScript types
└── src/__tests__/         contrast matrix test (R-ds-8), shape tests

apps/mobile/src/theme      React runtime binding (React stays out of tokens)
├── ThemeProvider.tsx      context + persistence + OS listeners
├── useTheme.ts            hook
├── createStyles.ts        StyleSheet factory + per-theme memo cache
└── haptics.ts             convention-mapped helpers over expo-haptics
```

Rationale: `@gogo/tokens` is importable by anything (server-rendered emails,
a future web app, tests) because it is pure data + one pure function. The
bartling ThemeProvider concept (runtime-swappable theme source) ports as: CSS
`<link>` swap → **context value swap**; CSS variables → **semantic token
object**; `.dark` class → **`buildTheme('dark', …)` output**.
...
**Layer 1 — ramps.** 11-step ramps keyed `50…950`. One `neutral` ramp (true
gray, may start from bartling's neutral values), fixed `success` (emerald),
`warning` (amber), `danger` (red), `info` (blue) ramps, and one `accent` ramp
**per accent theme** (the only ramp that swaps).
...
**Layer 2 — semantic tokens.** Components consume ONLY these. Mapping tables
(one for light, one for dark) select ramp steps:

```ts
interface SemanticColors {
  bg: {
    screen: string;        // light: neutral-50   dark: neutral-950
    surface: string;       // cards/sheets — light: white        dark: neutral-900
    surfaceRaised: string; // elevated sheets — light: white     dark: neutral-800
    inset: string;         // inputs, wells — light: neutral-100 dark: neutral-800
    scrim: string;         // modal backdrop rgba
  };
  text: {
    primary: string;       // neutral-900 / neutral-50
    secondary: string;     // neutral-600 / neutral-300
    muted: string;         // neutral-500 / neutral-400 (must still pass 4.5:1 on surface)
    inverse: string;
    onAccent: string;      // text on accent.solid — validated per accent theme
    accent: string;        // links / active tint — accent-600 / accent-400
  };
  border: { subtle: string; default: string; strong: string; focus: string };
  accent: {
    solid: string;         // accent-600 / accent-500
    solidPressed: string;  // accent-700 / accent-400
    subtleBg: string;      // accent-50 at ~opacity / dark translucent — pills, selected rows
    subtleBorder: string;
    onSolid: string;       // = text.onAccent
  };
  status: Record<'success' | 'warning' | 'danger' | 'info',
    { fg: string; bg: string; border: string }>;
  interactive: { pressedOverlay: string; disabledBg: string; disabledText: string };
}
```

**Layer 3 — accent themes** (the re-skinnable part):

```ts
interface AccentThemeDef {
  name: string;                          // stable key, kebab: 'default', 'coastal'
  label: string;                         // display name
  ramp: ColorRamp;                       // the accent ramp
  overrides?: DeepPartial<SemanticColors>; // rare, per-scheme escape hatch
}
```

`buildTheme(scheme, accentDef)` composes ramps + mapping table + overrides
into a frozen `Theme`. Adding a theme = adding one `AccentThemeDef` to the
registry (R-ds-5). Exact hex values for ramps land in the implementation task
and must pass the R-ds-8 contrast matrix; the mapping table above is the
contract. Gradients (bartling used them heavily) are **out** for v1 —
minimalistic means flat surfaces + one accent; revisit only with a design pass.
...
```ts
type TypeRole = 'display' | 'title' | 'heading' | 'subheading'
              | 'body' | 'bodyStrong' | 'caption' | 'label' | 'mono';
...
elevation: Record<0 | 1 | 2 | 3 | 4, ElevationStyle>
```

Dark scheme reduces shadow reliance (shadows read poorly on dark) and leans on
`bg.surface` / `bg.surfaceRaised` separation instead — levels still exist but
opacity drops.
...
```ts
motion: {
  duration: { fast: 120, base: 200, slow: 300 },   // ms
...
skeleton shimmer 1200 ms loop. Reduce-motion (R-ds-11): shimmer → static
...
interface Theme {
  ...
  ramp: { neutral: ColorRamp; accent: ColorRamp; success: ColorRamp;
          warning: ColorRamp; danger: ColorRamp; info: ColorRamp };
...
- Persistence: MMKV keys `gogo.appearance`, `gogo.accentTheme`. MMKV is
  synchronous — read BEFORE first render so R-ds-4 (no flash) holds.
- Subscribes to `Appearance.addChangeListener` for R-ds-3.
- Theme objects are memoized per `(scheme, accent)` so context consumers can
  use reference equality.

**createStyles pattern** (R-ds-7) — the RN port of bartling's component
classes:
...
// in component
const s = useStyles(); // internally: useTheme() → per-Theme cache → StyleSheet.create once
```

`createStyles` caches the `StyleSheet.create` result in a
`WeakMap<Theme, styles>` — factories run once per theme, not per render.
Enforcement: an ESLint rule (`no-restricted-syntax` on color literals +
banning bare `StyleSheet.create` in `apps/mobile/src/(screens|features)`)
lands with the scaffold.

### 2.8 Haptics conventions (R-ds-13, R-ds-21)

Wrapper `haptics.ts` maps semantic events → expo-haptics calls; components
reference events, never raw calls.
...
interface ButtonProps {
  ...
  icon?: IconName; iconPosition?: 'leading' | 'trailing';
...
```
primary = `accent.solid`/`onSolid`; secondary = `bg.inset` + border; ghost =
transparent + `text.accent`; destructive = `status.danger`. Pressed state via
`interactive.pressedOverlay` (R-ds-13); loading per R-ds-14.
...
interface InputProps {
  ...
  placeholder?: string; helper?: string; error?: string;   // error state → border.focus=danger + helper slot
  leading?: ReactNode; trailing?: ReactNode;               // icons, clear button, currency prefix
  secureTextEntry?: boolean; multiline?: boolean;
  keyboardType?: KeyboardTypeOptions; autoComplete?: string; returnKeyType?: string;
  testID: string;
}
```
...
'warning' | 'danger' | 'info'`, `size?: 'sm' | 'md'`. Uses `status.*` /
`accent.subtle*` pairs. Non-interactive (no testID requirement). Used for:
booking status idea/planned/booked, parse status, member roles, offline pill.
...
receives router state/navigation. Active tint `accent.solid`, inactive
...
| DS-2 | Semantic mapping tables (light/dark) + accent theme registry + `buildTheme()`; **contrast matrix test** over every declared pair × scheme × accent. | R-ds-5, R-ds-6, R-ds-8 |
| DS-3 | Mobile theme runtime: ThemeProvider, `useTheme`, MMKV persistence, synchronous boot read, OS appearance listener. | R-ds-1..4, R-ds-6 |
