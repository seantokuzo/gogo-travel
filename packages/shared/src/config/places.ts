 ...
 * every row sharing any of them, O(10^5–10^6) heap rechecks + similarity()
 * calls per page, re-paid on every cursor step. ≥ 4 chars ⇒ ≥ 5 trigrams
 * and real selectivity. 2–3-char typeahead stays legal WHEN a geo bound is
 * present (the map surface always has one) — the lat/lng window bounds the
 * candidate set instead.
 *
 * B-7 follow-up (Sean's ruling, 2026-09-14, R-places-28): a sub-floor
 * TEXT-ONLY query is no longer REJECTED (`PlaceSearchQuerySchema` stopped
