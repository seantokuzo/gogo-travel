...
export interface E2eCleanupDeps {
  ...
  now?: () => Date;
  /** One line per notable event — defaults to `console.warn` (a maintenance script, not a request handler; the server no-`console.log` rule targets request-path code). */
...
async function deleteAllFixtureTrips(
  ...
  if (fixtureUsers.length === 0) return 0;
  ...
  if (ownedByFixture.length === 0) return 0;
  ...
  for (const { tripId, ownerId } of ownedByFixture) {
    ...
    const result = await deleteTripCore(db, tripId, ownerId);
    if (result.deleted) {
      deletedCount += 1;
      log(`[e2e-cleanup] deleted all-fixture trip ${tripId} (owner ${ownerId})`);
    }
  }
  return deletedCount;
}
...
async function deleteRemainingFixtureUsers(
  deps: Pick<E2eCleanupDeps, "db" | "appleCredentialsKey" | "now">,
  fixtureUsers: readonly FixtureUserRow[],
  log: (line: string) => void,
  ...
  for (const user of fixtureUsers) {
    try {
      ...
      if (result.status === "deleted") {
        deleted += 1;
        log(`[e2e-cleanup] deleted fixture user ${user.userId} (${user.appleSub})`);
      }
    } catch (error) {
...
async function runOnePass(
  ...
  const tripsDeleted = await deleteAllFixtureTrips(deps.db, fixtureUsersBeforeTrips, log);
  ...
  return { tripsDeleted, usersDeleted: deleted, skipped, failures };
}
