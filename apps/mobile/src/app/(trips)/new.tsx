 ...
 * 3. Dates — REQUIRED range (resolved Gate 2), start/end `YYYY-MM-DD`
 *    fields under one `trip-new-input-dates` control. (A native calendar
 *    range picker needs a dependency the DS doesn't ship — typed fields are
 *    the v1 control; swapping the input surface later is additive.)
 *
...
export default function TripNewScreen() {
  ...
  // form. A not-yet-loaded profile is fine — omitting the field lets the
  // server default 'USD'; the currency stays editable in trip settings
  // until the first expense locks it (API §3.6).
  const me = useMe();
  const homeCurrency = me.data?.prefs.home_currency;
  ...
  const submit = useCallback(() => {
    ...
    if (startDate === "") errors.start_date = "Required (YYYY-MM-DD).";
    if (endDate === "") errors.end_date = "Required (YYYY-MM-DD).";
  ...
  }, [name, selectedPlace, startDate, endDate, homeCurrency, createTrip, router]);
        ...
        trailing={[
          {
            icon: "close",
            label: "Cancel",
            // router.back() routes through beforeRemove — dirty forms get
            // the discard Confirm, clean forms just dismiss (R-tripui-8).
            onPress: () => router.back(),
