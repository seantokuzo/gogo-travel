...
**Never drop `wrapper_pid` when rewriting the object**, even though nothing above tells you to touch it. `status` and `resume` both depend on it to tell a live wrapper from a dead one; drop it and `status` wrongly reports `INTERRUPTED` on a chain that's still very much running, which is exactly the setup for the `stop`-under-a-live-wrapper data-loss path in §15.
...
| `status` reports `INTERRUPTED`                                               | Wrapper process died with no sentinel (machine slept, terminal closed, killed) — OR `wrapper_pid` was dropped from `state.json` by a hand-edit that didn't follow §13's warning, while the wrapper is actually still running. | Before running `stop` (`rm -rf .loop/`), confirm the wrapper is ACTUALLY gone — check for a running `claude -p` chained to this repo (e.g. `ps aux \| grep run-loop.sh`) — not just that `status` says so. If it's genuinely gone: `resume` needs a real `pivot`/`blocked` sentinel, so it won't fire here. Read `.loop/log.txt`/`state.json` for the last known task, then either hand-write a `pivot` describing where it stopped and `resume --prompt` from it, or `stop` then `start` fresh with a prompt describing where to pick up. If it's still running: do NOT `stop` — that deletes `.loop/` out from under it. Let it finish (or add the missing `wrapper_pid` back to `state.json` so `status` reports correctly). |
